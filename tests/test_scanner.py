...
class TestGenerateFixCanonical:
    ...
    def test_extras_and_marker_still_fixed_when_not_compliant(self):
        entries = [
            DepEntry(
                repo="a",
                raw="mcp[server]>=1.0; python_version<'3.11'",
                name="mcp",
                specifiers=">=1.0",
                extras=["server"],
...
class TestFindConflictsActionable:
    ...
    def test_min_floor_helper(self):
        ...
        assert min_floor(">=8.1.0") == min_floor(">=8.1.0")
        # upper bounds don't contribute a floor
        assert min_floor(">=8.1.0,<9.0") == min_floor(">=8.1.0")
        # strict > contributes its floor; == contributes its pin as a floor
        assert min_floor(">8.0") == min_floor(">=8.0")
